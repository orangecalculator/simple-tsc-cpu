instructionstr = """        memory[16'h0] <= 16'h9023; memory[16'h1] <= 16'h1; memory[16'h2] <= 16'hffff; memory[16'h3] <= 16'h0;
...
instructionlist = instructionstr.split(';')
instructionlist.pop()
instructionlistdec = [0] * len(instructionlist)

for i in range(len(instructionlist)):
	instructionlistdec[i] = (instructionlist[i].split('<='))[1]

def hexdecode(s):

	possibledigit = [str(num) for num in range(10)]+[chr(unicode) for unicode in range(97,103)]

	possibledecode = ["{:d}{:d}{:d}{:d}".format(num//8,((num%8)//4),((num%4)//2),((num%2))) for num in range(16)]
	decodebin = ""

	for i in range(len(s)-1,-1,-1):
		if(s[i] in possibledigit):
			if(ord('0')<=ord(s[i])<=ord('9')):
				currdigit = int(s[i])
			else:
				currdigit = ord(s[i]) - ord('a') + 10

			decodebin = possibledecode[currdigit] + decodebin
		else:
			break

	return "{:0>16s}".format(decodebin)

instructionlistbin = [hexdecode(hexcode) for hexcode in instructionlistdec]

def reversecompile(bincode):
	opcode = bincode[0:3]
	if(opcode=="1111"):
		type = 'R'
	...
	if type is 'R':
		...
		functdict = {0:'ADD',1:'SUB',2:'AND',3:'ORR',4:'NOT',5:'TCP',6:'SHL',7:'SHR',28:'WWD',25:'JPR',26:'JRL',29:'HLT'}

		inst = functdict[funct]
		rs = int(bincode[4:6],2)
		rt = int(bincode[6:8],2)
		rd = int(bincode[8:10],2)

		return "{} {} {} {}".format(inst,str(rd),str(rs),str(rt))
	elif type is 'I':
		opcode = int(opcode,2)
		...
		return "{} {} {}".format(opcodedict[opcode],str(rt),str(rs),bincode[8:])
	else:
		opcode = int(opcode,2)
		opcodedict={9:'JMP',10:'JAL'}

		return "{} {}".format(opcodedict[opcode],bincode[4:])

instructionlistrevcomp = [reversecompile(bincode) for bincode in instructionlistbin]

for i in range(len(instructionlistrevcomp)):
	print(instructionlistrevcomp[i])
